function [a, Phi] = POD_1D_NonUniformGrid_DirectMethod(xloc, w, u, x, compression)
    %% Uses direct method, not snapshot method (ok for small space)
...
% phi is the eigen modes
    ...
    Psi(:,:) = eigvect(:,M:-1:1); % ASSUMES AN ORDER FROM MATLAB's eig function
    w2 = w*ones(1,M);
    Phi = Psi ./ sqrt(w2);
